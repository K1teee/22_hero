...
void Send_USB(float Roll,float Pitch,float Yaw)
{
	usb_tx.hander=0x5A;
	usb_tx.detect=1;
	usb_tx.id = 0x01;
	...
	usb_tx.yaw = Yaw;
	usb_tx.crc16 = 0xffff;
	memcpy(Buffer,&usb_tx,sizeof(usb_tx));
	Append_CRC16_Check_Sum(Buffer,sizeof(usb_tx));
	CDC_Transmit_FS(Buffer,sizeof(usb_tx));
}
    ...
    uint8_t res;
		for(uint16_t j=0;j<Len;j++)
		{
			...
			{   
				...
				//go_left
				vision_sent.chassis.target_y =  (float)((int16_t)(receive_data[7]<<8|receive_data[6]));
				//turn_left
				vision_sent.chassis.target_z =  (float)((int16_t)(receive_data[9]<<8|receive_data[8]))/100.0f;

				mode_flag = (int16_t)(receive_data[11]<<8|receive_data[10]);	//和定义的不一致，改can的时候一并改了

				top_flag = (int16_t)(receive_data[13]<<8|receive_data[12]);	//和定义的不一致，改can的时候一并改了
					//shoot_flag
			    shoot_flag = (int16_t)(receive_data[15]<<8|receive_data[14]);	//同上
...
void UPPER_COMM_TX(void)
{
	Send_USB(gimbal_r.IMU_actualangle,gimbal_p.IMU_actualangle,gimbal_y.IMU_actualangle);//发送roll,pit,yaw的数据。
